package com.thezayin.editor.utils.other.anim
...
object AnimUtils {
    ...
    fun fadeInThenFadeOut(
        fadeInMillis: Int = 200, fadeOutMillis: Int = 200
    ) = fadeIn(
...
fun enterTransition(): EnterTransition {
    return slideIn(animationSpec = tween(
...
fun exitTransition(): ExitTransition {
    return slideOut(animationSpec = tween(
        durationMillis = TRANSITION_DURATION,
        easing = FastOutSlowInEasing
    ),
        targetOffset = { IntOffset(-1 * (it.width / 2), 0) }) + fadeOut(
        animationSpec = tween(durationMillis = TRANSITION_DURATION, easing = FastOutSlowInEasing),
    )
}
