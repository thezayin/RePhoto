...
import com.thezayin.editor.utils.textMode.colorList.ColorListFullWidth
...
fun TextEditorLayout(
    ...
    val viewModel: TextEditorViewModel = koinInject()
    ...
    LaunchedEffect(key1 = Unit) {
        ...
        Timber.tag("TEST_editor").d("TextEditorLayout: id = ${initialState.textStateId}")
    ...
    val showPlaceHolder by remember(key1 = editorState.text.isBlank()) {
        Log.e(
            "TEST_text_mode",
            "TextEditorLayout: showPlaceHolder = ${editorState.text.isBlank()}",
        )
        mutableStateOf(editorState.text.isBlank())
    }

    val onColorItemClicked = remember<(Int, Color) -> Unit> {
        { index, color ->
            viewModel.onEvent(TextEditorEvent.SelectTextColor(index, color))
        }
    }

    val onTextAlignItemClicked = remember<(Int, TextAlign) -> Unit> {
        { index, textAlign ->
            viewModel.onEvent(TextEditorEvent.SelectTextAlign(index, textAlign))
        }
    }

    val onTextFieldValueChange = remember<(TextFieldValue) -> Unit> {
        { mValue ->
